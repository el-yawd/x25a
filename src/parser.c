...
void parse_REST_DECLS(){
    // Check if next token can start a declaration
    if(curtok.type == T_ID || curtok.type == T_KW_LEIA || curtok.type == T_KW_ESCREVA ||
       curtok.type == T_KW_SE || curtok.type == T_KW_FACA) {
        ...
        // Trailing comma before block end
        expect(T_COMMA);
        // Don't continue if at block terminator
        if(!is_block_end()) {
            parse_DECL();
            parse_REST_DECLS();
        }
    }
    // else: epsilon - no more declarations
}
